...
export async function loadWorld(channel:"draft"|"published"="draft"):Promise<WorldPayload>{
  const response = await fetch(`${ENDPOINT}?channel=${channel}&include=archetypes,assets`, { cache:"no-store" });
...
export function publicAssetUrl(filePath:string){
  return `https://umvmilulnqnmeqvfoxxc.supabase.co/storage/v1/object/public/hamster-assets/${encodeURIComponent(filePath).replaceAll("%2F","/")}`;
}
