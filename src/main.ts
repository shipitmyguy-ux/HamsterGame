...
function bindHold(button:HTMLElement,key:keyof typeof movement){
  ...
  button.addEventListener("pointercancel",off);button.addEventListener("lostpointercapture",()=>movement[key]=false);
}
...
class GameScene extends Phaser.Scene{
  ...
  openRoom(id:string){
    ...
    setStatus(`${room.name} · draft v${this.payload.version} · ${room.entities?.length||0} objects`);
  }
  ...
  update(){
    ...
    const speed=190;
    let dx=0,dy=0;
    if(movement.left||this.arrows?.left.isDown||this.wasd?.A.isDown)dx--;
    if(movement.right||this.arrows?.right.isDown||this.wasd?.D.isDown)dx++;
    if(movement.up||this.arrows?.up.isDown||this.wasd?.W.isDown)dy--;
    if(movement.down||this.arrows?.down.isDown||this.wasd?.S.isDown)dy++;
...
function guessEmoji(a:string){
  if(a.includes("chair"))return "🪑";if(a.includes("bed"))return "🛏️";if(a.includes("tree"))return "🌳";
  if(a.includes("rock"))return "🪨";if(a.includes("wheel"))return "🎡";if(a.includes("house"))return "🏠";
  if(a.includes("door"))return "🚪";if(a.includes("flower"))return "🌻";if(a.includes("rug"))return "🟫";
  if(a.includes("pickup"))return "✨";return "📦";
}

function buildRoomBar(scene:GameScene,current:string){
  roomBar.innerHTML="";
  Object.values(scene.payload.world.rooms).forEach(room=>{
    const b=document.createElement("button");b.type="button";b.textContent=room.name;
...
function bindActions(scene:GameScene){
  ...
  document.querySelector<HTMLButtonElement>("#build")!.onclick=()=>setStatus("Build mode is connected next; world edits already live in the backend.");
}
